...
class Database:
    ...
    def get_close_appointments(self, user, date):
        ...
        if date == 'all':
            ...
            appointments = collection.find({user_type: user['userid']})
            user_appointments = [app for app in appointments if (
                int(date) - int(app['date_key'])) < 10]
    ...
    def get_user_appointments(self, user, date):
        ...
        if date == 'all':
            ...
            appointments = collection.find({user_type: user['userid']})
            user_appointments = [app for app in appointments if int(
                date) == int(app['date_key'])]
    ...
    def search_my_patient(self, userid, doc):
    ...
    def search_user(self, infor, user_type, userid):
        ...
        if user_type == 'patient':
            ...
            for patient in found_users:
                found_patients = collection.find({})

                found_patients = [pat for pat in found_patients]

                single_patient = [
                    p for p in found_patients if p['userid'] == patient['userid']]

                try:
                    patient_index = found_patients.index(single_patient[0])

                    patient_data = found_patients[patient_index]
                    if userid in patient_data['doctor_allowed_view']:
                        can_view = True
                    if userid in patient_data['doctor_allowed_update']:
                        can_view = True
                except Exception as ex:
                    print('Error(searching patient) : ', ex)
    ...
    def save_block(self, block: blockchain.Block):
        ...
        for transaction in block.transactions:
            ...
            transaction2save = {"type": transaction.type_,
